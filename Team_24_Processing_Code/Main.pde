...
void setup() {
  ...
  for (int i = 0; i < values.size(); i++){
    ...
    println(data.MKT_CARRIER + " " + data.TOTAL_DIST);
  }
  println("CARRIER-COUNCANCELLED-COUNTDIVERTED");
  ArrayList<PieDataPoint> values2 = queries.getRowsPieChart();
  for (int i = 0; i < values2.size(); i++){
    PieDataPoint data = values2.get(i);
    println(data.MKT_CARRIER + " " + data.COUNT_CANCELLED + " " + data.COUNT_DIVERTED);
  }
